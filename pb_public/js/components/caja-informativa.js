...
class CajaInformativa extends ComponenteBase {
  ...
  constructor() {
    super();
    this.tipo = "texto";
    (this.titulo, this.valor, (this.color = ""));
    this.estaHecho = false;
  }

  renderizarTexto() {
    return html` <p class="subtitle is-6">${this.valor}</p> `;
  }

  renderizarTextoLargo() {
    return html` <p>${this.valor}</p> `;
  }

  renderizarProgreso() {
    return html`
      <p class="subtitle is-6 mb-2">${this.actual} / ${this.max}</p>
  ...
  renderizarValorVacio() {
    return html` <p class="subtitle is-6 has-text-dark-soft">—</p> `;
  }
  ...
  renderizarPuntos() {
    ...
    return html`
      <p class="subtitle is-6">${serieVacia.map(() => circulo)}</p>
    `;
  }
  ...
  render() {
    ...
    switch (this.tipo) {
      ...
      case "texto-largo":
        parrafoValor = this.renderizarTextoLargo();
        break;
    ...
    let clasesAdicionales =
      `has-background-${this.color}-soft ` +
      (this.margenInferior === undefined ? "" : `mb-${this.margenInferior}`);
    let clasesTituloAdicionales = "";

    if (tituloEstaApagado) {
      clasesTituloAdicionales += "has-text-dark-soft ";
    }

    if (this.tipo === "texto-largo") {
      clasesTituloAdicionales += "mb-2 ";
    }

    return html`
      <div class="box ${clasesAdicionales}">
        <p class="title is-5 ${clasesTituloAdicionales}">${this.titulo}</p>
